...
const block = getElement<HTMLDivElement>('moving-block');
const ball = getElement<HTMLDivElement>('white-ball');
const boundary = getElement<HTMLDivElement>('boundary');
...
function update() {
  ...
  if (keysPressed['ArrowUp']) velocityY -= acceleration;
  if (keysPressed['ArrowDown']) velocityY += acceleration;
  if (keysPressed['ArrowLeft']) velocityX -= acceleration;
  if (keysPressed['ArrowRight']) velocityX += acceleration;
